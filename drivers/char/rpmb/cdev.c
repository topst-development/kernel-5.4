		...
		struct rpmb_info_cmd __user *ptr)
{
	...
	rpmbd->ufs_info = memdup_user(u64_to_ptr(cmd.desc_info_ptr),
			sizeof(struct ufs_desc_info));
	...
	if (copy_to_user(u64_to_ptr(cmd.desc_info_ptr), rpmbd->ufs_info,
				sizeof(struct ufs_desc_info))) {
		ret = -EFAULT;
		goto out;
	}
	...
	dev_dbg(&rdev->dev, "%s : %s\n", __func__,
			rpmbd->ufs_info->model);
	dev_dbg(&rdev->dev, "%s : 0x%x\n", __func__,
			rpmbd->ufs_info->bRPMB_ReadWriteSize);
	dev_dbg(&rdev->dev, "%s : 0x%x\n", __func__,
			rpmbd->ufs_info->bRPMBRegion0Size);

...
//Decoder
typedef int (*tccfp_jpu_dec_t)(int, codec_handle_t*, void*, void*);
tccfp_jpu_dec_t tcc_jpu_dec;

//Encoder
#	if DEFINED_CONFIG_VENC_CNT_1to16
	typedef int (*tccfp_jpu_enc_t)(int, codec_handle_t*, void*, void*);
	tccfp_jpu_enc_t tcc_jpu_enc;
#	endif

typedef struct st_jpu_func_t {
	int check_code1;
	int (*tccfp_jpu_dec)(int, codec_handle_t*, void*, void*);
	int check_code2;
	int (*tccfp_jpu_enc)(int, codec_handle_t *, void *, void *);
	int check_code3;
	int (*tccfp_jpu_dec_esc)(int, codec_handle_t *, void *, void *);
	int (*tccfp_jpu_dec_ext)(int, codec_handle_t *, void *, void *);
	int check_code4;
} st_jpu_func;
...
#	endif
	...
	filp = filp_open("/proc/jpu", O_CREAT | O_RDWR, 0644);
...
#	endif
	...
	V_DBG(VPU_DBG_INFO, "JPU CheckCode %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n",
			GET_FOURCC_1(stJPUFunc->check_code1),
			GET_FOURCC_2(stJPUFunc->check_code1),
			GET_FOURCC_3(stJPUFunc->check_code1),
			GET_FOURCC_4(stJPUFunc->check_code1),
			GET_FOURCC_1(stJPUFunc->check_code2),
			GET_FOURCC_2(stJPUFunc->check_code2),
			GET_FOURCC_3(stJPUFunc->check_code2),
			GET_FOURCC_4(stJPUFunc->check_code2),
			GET_FOURCC_1(stJPUFunc->check_code3),
			GET_FOURCC_2(stJPUFunc->check_code3),
			GET_FOURCC_3(stJPUFunc->check_code3),
			GET_FOURCC_4(stJPUFunc->check_code3),
			GET_FOURCC_1(stJPUFunc->check_code4),
			GET_FOURCC_2(stJPUFunc->check_code4),
			GET_FOURCC_3(stJPUFunc->check_code4),
			GET_FOURCC_4(stJPUFunc->check_code4)
			);

	if(ret == 0 &&
			((CHECK_CODE_01 | stJPUFunc->check_code1) == CHECK_CODE_01) &&
...
static int check_jpu_access_addr_valid(void)
{
	...
	{
		V_DBG(VPU_DBG_ERROR, "JPU CheckCode %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n",
				GET_FOURCC_1(stJPUFunc->check_code1),
				GET_FOURCC_2(stJPUFunc->check_code1),
				GET_FOURCC_3(stJPUFunc->check_code1),
				GET_FOURCC_4(stJPUFunc->check_code1),

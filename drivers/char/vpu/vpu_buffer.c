...
int _vmem_init_memory_info(void)
{
...
#if DEFINED_CONFIG_VENC_CNT_1to16
	{
		...
		V_DBG(VPU_DBG_MEMORY, "size of enc_main: %lu for 0x%lx",
...
void vdec_get_instance(int *nIdx)
{
	mutex_lock(&mem_mutex);
	{
		int idx = *nIdx;
		int i, nInstance = -1;
		if (vmem_get_free_memory(idx + VPU_DEC) > 0) {
			if (vmem_get_vdec_used(idx) == 0) {
				nInstance = idx;
			}
			else {
				for (i = 0; i < VPU_INST_MAX; i++) {
					if (vmem_get_vdec_used(i) == 0) {
						nInstance = i;
						break;
					}
				}
		...
		if (vmem_set_vdec_used(nInstance, 1) < 0)
			V_DBG(VPU_DBG_ERROR,
...
void vdec_check_instance_available(unsigned int *nAvailable_Inst)
{
	mutex_lock(&mem_mutex);
	{
		...
		for (i = 0; i < VPU_INST_MAX; i++) {
			if (vmem_get_free_memory(i + VPU_DEC) > 0) {
				if (vmem_get_vdec_used(i) == 0) {
					nAvailable_Instance++;
				}
			}
...
void vdec_clear_instance(int nIdx)
{
	mutex_lock(&mem_mutex);
	{
		if (vmem_set_vdec_used(nIdx, 0) < 0)
			V_DBG(VPU_DBG_ERROR,

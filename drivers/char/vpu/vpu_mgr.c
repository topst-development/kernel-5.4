...
//Decoder
typedef int (*tccfp_vpu_dec_t)(int, codec_handle_t*, void*, void*);
tccfp_vpu_dec_t tcc_vpu_dec;
typedef int (*tccfp_vpu_dec_esc_t)(int, codec_handle_t*, void*, void*);
tccfp_vpu_dec_esc_t tcc_vpu_dec_esc;
typedef int (*tccfp_vpu_dec_ext_t)(int, codec_handle_t*, void*, void*);
tccfp_vpu_dec_ext_t tcc_vpu_dec_ext;

//Encoder
#	if DEFINED_CONFIG_VENC_CNT_1to16
	typedef int (*tccfp_vpu_enc_t)(int, codec_handle_t*, void*, void*);
	tccfp_vpu_enc_t tcc_vpu_enc;
#	endif

typedef struct st_vpu_func_t {
	int check_code1;
	int (*tccfp_vpu_dec)(int, codec_handle_t*, void*, void*);
	int check_code2;
	int (*tccfp_vpu_enc)(int, codec_handle_t *, void *, void *);
	int check_code3;
	int (*tccfp_vpu_dec_esc)(int, codec_handle_t *, void *, void *);
	int (*tccfp_vpu_dec_ext)(int, codec_handle_t *, void *, void *);
	int check_code4;
} st_vpu_func;
...
#	endif
	...
	filp = filp_open("/proc/vpu", O_CREAT | O_RDWR, 0644);
...
#	endif
	...
	V_DBG(VPU_DBG_INFO, "VPU CheckCode %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n",
	...
	if(ret == 0 &&
			((CHECK_CODE_01 | stVPUFunc->check_code1) == CHECK_CODE_01) &&
...
static int _vmgr_process(vputype type, int cmd, long pHandle, void *args)
{
	...
	if (type < VPU_ENC) {
		...
		case VPU_DEC_DECODE_KERNEL:
		{
			...
			V_DBG(VPU_DBG_INFO,
					"Dec-%d :: In => 0x%x - 0x%x, %d, 0x%x - 0x%x, %d,"\
					" flag: %d / %d / %d",
					type,
			...
			V_DBG(VPU_DBG_INFO,
					"Dec-%d :: Out => ret[%d] !! PicType[%d],"\
					" OutIdx[%d/%d], OutStatus[%d/%d]",
					type, ret,

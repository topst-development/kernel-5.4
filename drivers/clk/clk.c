...
#ifdef CONFIG_ARCH_TCC
static int __clk_set_src(struct clk_core *core, unsigned long src,
		unsigned long div)
{
	int ret = 0;
	...
	if (core) {
		if (!WARN_ON(IS_ERR(core))) {
			if (core->ops->set_src) {
				ret = core->ops->set_src(core->hw, src, div);
			} else {
				ret = -1;
			}
		}
...
/**
 * clk_set_src - Specify a new clock source for clk and divider
 * 		 This function only work for PCLKDIVCTRL or PCLKCDOCTRL
 * @clk: The clk whose rate is being changed
 * @src: The new clock source's src value for clk
 * @div: Divider value for new source clock
 * 	 (div > 0) - Divider mode
 * 	 (div > 1) - DCO mode
 */
int clk_set_src(struct clk *clk, unsigned long src, unsigned long div)
{
	int ret = 0;
	unsigned long flags;
	...
	if (!clk) {
		...
		if (clk->core) {
			ret = __clk_set_src(clk->core, src, div);
			if (ret == 0) {

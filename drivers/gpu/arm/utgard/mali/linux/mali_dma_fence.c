...
#endif
	MALI_DEBUG_ASSERT_POINTER(dma_fence_context);
	MALI_DEBUG_ASSERT_POINTER(dma_resv);

	/* Get all the shared/exclusive fences in the reservation object of dma buf*/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0) // TCC
	ret = dma_resv_get_fences_rcu(dma_resv, &exclusive_fence,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0) // TCC
void mali_dma_fence_add_reservation_object_list(struct dma_resv *dma_resv,
		struct dma_resv **dma_resv_list,
		u32 *num_dma_resv)
{
	u32 i;

	MALI_DEBUG_ASSERT_POINTER(dma_resv);
	MALI_DEBUG_ASSERT_POINTER(dma_resv_list);
	MALI_DEBUG_ASSERT_POINTER(num_resv_list);

	for (i = 0; i < *num_dma_resv; i++) {
		if (dma_resv_list[i] == dma_resv)
			return;
	}

	dma_resv_list[*num_dma_resv] = dma_resv;
	(*num_dma_resv)++;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0) // TCC
int mali_dma_fence_lock_reservation_object_list(struct dma_resv **dma_resv_list,
		u32  num_dma_resv, struct ww_acquire_ctx *ww_actx)
{
	u32 i;

	struct dma_resv *dma_resv_to_slow_lock = NULL;
...
again:
	for (i = 0; i < num_dma_resv; i++) {
		...
		if (dma_resv_list[i] == dma_resv_to_slow_lock) {
			dma_resv_to_slow_lock = NULL;
			continue;
		}

		ret = ww_mutex_lock(&dma_resv_list[i]->lock, ww_actx);
		...
		if (ret < 0) {
			...
			while (i > 0) {
				i--;
				ww_mutex_unlock(&dma_resv_list[i]->lock);
			}

			if (NULL != dma_resv_to_slow_lock)
				ww_mutex_unlock(&dma_resv_to_slow_lock->lock);

			if (ret == -EDEADLK) {
				dma_resv_to_slow_lock = dma_resv_list[slow_lock_index];
				ww_mutex_lock_slow(&dma_resv_to_slow_lock->lock, ww_actx);
				goto again;
			}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0) // TCC
void mali_dma_fence_unlock_reservation_object_list(struct dma_resv **dma_resv_list,
		u32 num_dma_resv, struct ww_acquire_ctx *ww_actx)
{
	u32 i;

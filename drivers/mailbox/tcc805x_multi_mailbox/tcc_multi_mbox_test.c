...
struct mbox_test_device {
	...
	struct timespec64 send_time;
	struct timespec64 txd_time;
	struct timespec64 ack_time;
};
...
static int mbox_test_create_dev(struct platform_device *pdev, struct mbox_test_device *mbox_test_dev);
...
static void mbox_test_receive_ack_cmd(void *device_info, struct tcc_mbox_data  *pMsg)
{
	int ret = -1;
	struct mbox_test_device *mbox_test_dev = (struct mbox_test_device *)device_info;
	struct timespec64 send_ts, ack_ts;
	...
	if ((pMsg != NULL) && (mbox_test_dev != NULL)) {
		...
		ktime_get_real_ts64(&mbox_test_dev->ack_time);

		send_ts = timespec64_sub(mbox_test_dev->txd_time, mbox_test_dev->send_time);
		ack_ts = timespec64_sub(mbox_test_dev->ack_time, mbox_test_dev->send_time);

		dprintk(mbox_test_dev->dev, "mbox send time(%ld)usec, ack time(%ld)usec\n", send_ts.tv_nsec, ack_ts.tv_nsec);
...
static int mbox_test_loopback(struct mbox_test_device *mbox_test_dev)
{
	...
	if (mbox_test_dev != NULL) {
		...
		mbox_test_dev->send_msg.cmd[4] = 0x12345678;
		...
		mbox_test_dev->send_msg.cmd[3] = 0x8888AAAA;
		mbox_test_dev->send_msg.cmd[4] = 0x12345678;
		mbox_test_dev->send_msg.cmd[5] = 0x9ABCDEF0;
...
static int mbox_test_send_message(struct mbox_test_device *mbox_test_dev, struct tcc_mbox_data *msg)
{
	...
	if ((mbox_test_dev != NULL) && (msg != NULL)) {
		...
		ktime_get_real_ts64(&mbox_test_dev->send_time);
		ret = mbox_send_message(mbox_test_dev->mbox_ch, msg);
		ktime_get_real_ts64(&mbox_test_dev->txd_time);
		if (ret >= 0)
			ret = 0;
		else
			ret = -1;
	...
	if ((pdev != NULL) && (mbox_test_dev != NULL)) {
		...
		if (IS_ERR(mbox_test_dev->class)) {
			...
			if (IS_ERR(mbox_test_dev->dev)) {
				ret = (int)PTR_ERR(mbox_test_dev->dev);
				...
				if (mbox_test_dev->mbox_ch != NULL)	{
					...
					ret = mbox_test_workqueue_initialize(mbox_test_dev);
				} else {
		...
		if (!mbox_test_dev) {
			...
			if (result != 0) {
				...
				if (result != 0) {
					...
					result = mbox_test_create_dev(pdev, mbox_test_dev);

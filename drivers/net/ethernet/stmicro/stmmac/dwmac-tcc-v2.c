...
#define GMAC_CONFIG_INTF_SEL_MASK	GENMASK(20, 18)
#define GMAC_CONFIG_INTF_GMII		(0x0 << 20)
#define GMAC_CONFIG_INTF_MII		(0x0 << 20)
#define GMAC_CONFIG_INTF_RGMII		(0x1 << 18)
#define GMAC_CONFIG_INTF_RMII		(0x1 << 20)
#define GMAC_SW_CONFIG_OFFSET   	(0x54)
#define GMAC_SW_CONFIG0_REG			(0x60 - GMAC_SW_CONFIG_OFFSET)
#define GMAC_SW_CONFIG1_REG			(0x64 - GMAC_SW_CONFIG_OFFSET)
#else
#define GMAC_CONFIG_INTF_SEL_MASK	GENMASK(22, 20)
#define GMAC_CONFIG_INTF_RGMII		(0x1 << 20)
#define GMAC_CONFIG_INTF_GMII		(0x0 << 20)
...
int tcc_dwmac_init(struct platform_device *pdev, void *priv)
{
	...
	if (gmac->phy_mode == PHY_INTERFACE_MODE_RMII
...
static struct tcc_dwmac *tcc_config_dt(struct platform_device *pdev)
{
	...
	gmac->use_ecid_mac_addr =
		of_property_read_bool(pdev->dev.of_node, "ecid-mac-addr");
	gmac->rmii_tx_clk_off = of_property_read_bool(
		pdev->dev.of_node, "telechips,rmii_tx_clk_off");
...
static int tcc_dwmac_probe(struct platform_device *pdev)
{
	...
	if (gmac->use_ecid_mac_addr) {
		dev_info(&pdev->dev, "get mac address from ecid\n");
		stmmac_res.mac = devm_kzalloc(&pdev->dev, ETH_ALEN, GFP_KERNEL);
		if (stmmac_res.mac) {
			if (tcc_read_mac_addr_from_ecid(stmmac_res.mac)) {
				dev_err(&pdev->dev,
					"can't read mac address from ecid\n");
			}
		} else {
			dev_err(&pdev->dev, "can't alloc mac address buffer\n");
		}
	}
	// TBD:

...
#if defined(CONFIG_SCSI_UFS_TCC) && defined(CONFIG_ARCH_TCC805X)
	reg = ufshcd_readl(hba, HCI_UTRL_NEXUS_TYPE);
	ufshcd_writel(hba, reg_setb(reg, lrbp->task_tag), HCI_UTRL_NEXUS_TYPE);
#endif
...
#if defined(CONFIG_SCSI_UFS_TCC) && defined(CONFIG_ARCH_TCC805X)
	reg = ufshcd_readl(hba, HCI_UTRL_NEXUS_TYPE);
	ufshcd_writel(hba, reg_clrb(reg, lrbp->task_tag), HCI_UTRL_NEXUS_TYPE);
#endif
	ufshcd_send_command(hba, tag);
	spin_unlock_irqrestore(hba->host->host_lock, flags);
		...
		struct utp_task_req_desc *treq, u8 tm_function)
{
...
#if defined(CONFIG_SCSI_UFS_TCC) && defined(CONFIG_ARCH_TCC805X)
	type = ufshcd_readl(hba, HCI_UTMRL_NEXUS_TYPE);
	switch (tm_function) {
		case UFS_ABORT_TASK:
		case UFS_QUERY_TASK:
			if(task_tag >= 0) {
				type = (unsigned int)(type | (1U << (unsigned)task_tag));
			}
			break;
	...
	ufshcd_writel(hba, type, HCI_UTMRL_NEXUS_TYPE);
#endif

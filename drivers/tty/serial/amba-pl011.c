...
#if defined(CONFIG_ARCH_TCC) && defined(CONFIG_PM_SLEEP)
			    ...
			    struct resource *mmiobase, int index)
{
	...
	ret = of_property_read_u32_index(dev->of_node, "config-reg", 2,	&uap->port.channel_num);
	if (ret){
	...
	if(uap->port.config_reg == NULL)
        ...
        if(uap->port.sc_np == NULL) {
                printk(KERN_INFO "[ERROR][PL011] no SC node\n");
        }
...
static void tcc_set_uart_port_cfg( struct uart_amba_port *uap, struct device *dev)
{
    ...
    if(node_detect != 0)
    {
		...
		for(i = 0; i < uap->port.channel_num; i++) {
			shift = (i%4)*8;
			tmp_portcfg = uap->port.phy_config_reg+((i/4u)*0x4u);
			portcfg_val = readl_relaxed(uap->port.config_reg+
					((i/4u)*0x4u));
			...
			if((uap->port.line!=i)&&(tmp_val==cfg_id)) {
...
#endif
				{
					pr_info("[ERROR][PL011] no SC firmware handle\n"
						);
                ...
                if(sc_fw_handle == NULL) {
                    ...
                    printk(KERN_INFO "[ERROR][PL011] no SC firmware handle\n");

...
#define TEST_VID 0x1a0a  /* TEST MODE VID */
...
{
	char *buf;
	u16 testmode, typeReq, wValue, wIndex, wLength;
	struct usb_hcd *hcd = bus_to_hcd(udev->bus);

	if (host_test_mode_val == 0)
		host_test_mode_val = le16_to_cpu(udev->descriptor.idProduct);

	if (le16_to_cpu(udev->descriptor.idProduct) == TEST_J) {
		pr_info("[INFO][USB] [%s] TEST_J, PID : 0x%04X\n",
				__func__, udev->descriptor.idProduct);
		testmode = 1;
	} else if (le16_to_cpu(udev->descriptor.idProduct) == TEST_K) {
		pr_info("[INFO][USB] [%s] TEST_K, PID : 0x%04X\n",
				__func__, udev->descriptor.idProduct);
		testmode = 2;
	} else if (le16_to_cpu(udev->descriptor.idProduct) == TEST_SE0_NAK) {
		pr_info("[INFO][USB] [%s] TEST_SE0_NAK, PID : 0x%04X\n",
				__func__, udev->descriptor.idProduct);
		testmode = 3;
	} else if (le16_to_cpu(udev->descriptor.idProduct) == TEST_PACKET) {
		...
		pr_err("[ERROR][USB] [%s] UNDEFINED TEST MODE!!!, PID : 0x%04X\n",
				__func__, udev->descriptor.idProduct);
	...
	hcd->driver->hub_control(hcd, typeReq, wValue, wIndex, buf, wLength);
...
{
	if (!hcd->usb_phy || !hcd->usb_phy->get_dc_level) {
	...
	return hcd->usb_phy->get_dc_level(hcd->usb_phy);
}

static int set_port_dc_level(struct usb_hcd *hcd, int level)
{
	if (!hcd->usb_phy || !hcd->usb_phy->set_dc_level) {
		pr_info("[INFO][USB] [%s:%d]PHY driver is needed\n",
			__func__, __LINE__);
	...
	return hcd->usb_phy->set_dc_level(hcd->usb_phy, level);
}
...
static int hub_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (hdev->level == hub_level) {
		dev_emerg(&intf->dev, "[EMERG][USB] Unsupported bus topology: hub nested too deep\n");
#else
...
static int usb_enumerate_device_otg(struct usb_device *udev)
{
	...
	(defined(CONFIG_TCC_EH_ELECT_TST) && defined(HCI_TPL_SUPPORT))
	/*
	...
	if (!udev->bus->is_b_host
		...
		if (port1 == bus->otg_port) {
			...
			if (err < 0) {
				 ...
				 */
#if defined(CONFIG_TCC_EH_ELECT_TST)
				dev_info(&udev->dev, "[INFO][USB] can't set HNP mode: %d\n",
						err);
#else
				...
				if (err < 0) {
					dev_info(&udev->dev, "[INFO][USB] can't set A-HNP SUPPORT: %d\n",
							err);
					bus->b_hnp_enable = 0;
				}
			}
...
#else
	pr_info("[INFO][USB] [%s:%d] For USB 3.0 electrical test, warm reset is unsupported.\n",
			__func__, __LINE__);
...
#if defined(CONFIG_TCC_EH_ELECT_TST)
				if (r == -ETIMEDOUT)
					dev_info(&udev->dev, "[INFO][USB] Device Not Connected/Responding (GetDescriptor timed out)\n");
#endif
				retval = -EMSGSIZE;
				continue;
			}
		...
		u16 portchange)
{
	...
	for (i = 0; i < SET_CONFIG_TRIES; i++) {
		...
		if (status < 0) {
			dev_dbg(&udev->dev, "[DEBUG][USB] [%s:%d]port init fail:dc level=0x%x\n",
			...
			if (dclevel > CONFIG_MIN_DC_VOLTAGE_LEVEL) {
				...
				dev_dbg(&udev->dev, "[DEBUG][USB] reset the hs dc voltage level\n");
				goto loop_disable;
			}
		...
		if (dclevel > CONFIG_MIN_DC_VOLTAGE_LEVEL) {
			...
			dev_dbg(&udev->dev, "[DEBUG][USB] set the stable hs dc voltage level(margin=1)\n");
		}
#endif /* CONFIG_DYNAMIC_DC_LEVEL_ADJUSTMENT */
		if (status < 0)
			goto loop;
		...
		/* TCC Embedded Host Electrical Test */
#if defined(CONFIG_TCC_EH_ELECT_TST)
		if (unlikely((host_test_mode_val >= TEST_SE0_NAK) &&
					(host_test_mode_val <=
					 SINGLE_STEP_SET_FEATURE))) {
			udev->descriptor.idVendor = cpu_to_le16(TEST_VID);
			udev->descriptor.idProduct =
				cpu_to_le16(host_test_mode_val);
			host_test_mode_val = 0;
		}

		if (le16_to_cpu(udev->descriptor.idVendor) == TEST_VID)
...
loop:
		...
		hub_free_dev(udev);
		usb_put_dev(udev);
		if ((status == -ENOTCONN) || (status == -EOPNOTSUPP)) {
#if defined(CONFIG_DYNAMIC_DC_LEVEL_ADJUSTMENT)
			set_port_dc_level(hcd, CONFIG_USB_HS_DC_VOLTAGE_LEVEL);
#endif /* CONFIG_DYNAMIC_DC_LEVEL_ADJUSTMENT */
			break;

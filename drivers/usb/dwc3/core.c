...
static void __dwc3_set_mode(struct work_struct *work)
{
	...
	case DWC3_GCTL_PRTCAP_HOST:
		...
		if (ret) {
			...
			if (IS_ENABLED(CONFIG_TCC_DWC3_PHY)) {
				if (dwc->usb2_phy) {
					dwc->usb2_phy->vbus_status = 1;
...
#endif /* !defined(CONFIG_TCC_DWC3_PHY) */
...
#else /* CONFIG_TCC_EH_ELECT_TST */
	if (dwc->dis_u3_susphy_quirk)
		reg &= ~DWC3_GUSB3PIPECTL_SUSPHY;
#endif /* !defined(CONFIG_TCC_EH_ELECT_TST */
...
#if defined(CONFIG_USB_DWC3_DUAL_ROLE)
	ret = device_create_file(&pdev->dev, &dev_attr_dwc3_drdmode);
	if (ret != 0) {
		dev_err(&pdev->dev,
				"[ERROR][USB] failed to create dwc3_drdmode\n");
		goto err5;
	}
#if defined(CONFIG_DWC3_DUAL_FIRST_HOST)
	dwc3_set_mode(dwc, USB_DR_MODE_HOST);
#elif defined(CONFIG_DWC3_DUAL_FIRST_DEV)
...
#if defined(CONFIG_USB_DWC3_DUAL_ROLE)
	device_remove_file(&pdev->dev, &dev_attr_dwc3_drdmode);
#endif /* CONFIG_USB_DWC3_DUAL_ROLE */

	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_TEST:
			if (test_mode > 0 && test_mode <= 5) {
				...
				pr_info("  1.Drive suspend on the root port\n");
				...
				pr_info("  2.Drive resume on the root port\n");

				xhci_set_link_state(xhci, ports[wIndex], XDEV_RESUME);

				/* Delay 100ms */
				mdelay(100);

				/* Clear the resume bit */
				xhci_set_link_state(xhci, ports[wIndex], XDEV_U0);

				/* Restore interrupts */
				writel(irq_pending, &xhci->ir_set->irq_pending);

				pr_info("End of HS_HOST_PORT_SUSPEND_RESUME\n");
				spin_lock_irqsave(&xhci->lock, flags);
			} else {
				goto error;
			}

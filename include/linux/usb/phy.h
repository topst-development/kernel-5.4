...
struct usb_phy {
	...
	void	(*select_mux)(struct usb_phy *phy, int is_mux);

	/* enable/disable phy status */
	int	(*set_state)(struct usb_phy *phy, int state);
	...
	int	(*get_dc_level)(struct usb_phy *phy);
	int	(*set_dc_level)(struct usb_phy *phy, unsigned int level);
#endif

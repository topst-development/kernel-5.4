			...
			uint32_t asrc_pair)
{
	...
	asrc->pair[asrc_pair].hw.fifo_in_size =
		(val == 0) ? TCC_ASRC_FIFO_SIZE_256WORD :
		(val == 1) ? TCC_ASRC_FIFO_SIZE_128WORD :
		(val == 2) ? TCC_ASRC_FIFO_SIZE_64WORD :
		(val == 3) ? TCC_ASRC_FIFO_SIZE_32WORD :
		(val == 4) ? TCC_ASRC_FIFO_SIZE_16WORD :
		(val == 5) ? TCC_ASRC_FIFO_SIZE_8WORD :
		(val == 6) ? TCC_ASRC_FIFO_SIZE_4WORD :
...
#define FIFO_IN_SIZE_FUNC(x)	\
	int pair##x##_fifo_in_size_set(struct snd_kcontrol *kcontrol, \
					struct snd_ctl_elem_value *ucontrol) \
	{ return fifo_in_size_set(kcontrol, ucontrol, x); } \
	int pair##x##_fifo_in_size_get(struct snd_kcontrol *kcontrol, \
					struct snd_ctl_elem_value *ucontrol) \
